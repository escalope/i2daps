...
public class DERAgentJADEAgent
		 extends JADEAgent {         
 
		 public DERAgentJADEAgent(){
		 super(new DERAgentProtocol(),new DERAgentInteractionLocks());
		 }
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		expectedInput=this.getMSM().getMentalEntityByType("fake_InquireDER_output_for_task_InformUserOfPowerStatus");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
			    ...
			    addExpectedInputs(tobject, "fake_InquireDER_output_for_task_InformUserOfPowerStatus","1",expectedInput);
             	addConsumedInput(to,"fake_InquireDER_output_for_task_InformUserOfPowerStatus",expectedInput);
			 }
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	...
  	   	boolean correctRole=conversation.getPlayedRole().equals ("DER");
       	...
       	if (tobject.getType().equals("InquireDER") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
		...
		boolean allEntitiesExist=true;
	      ...
	      allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
             ...
             expectedApp=(ingenias.jade.components.Application)getAM().getApplication("DERSensor");
             tobject.addApplication("DERSensor",expectedApp);
        ...
        /*     
	     ...
	     boolean alreadyExists=true;
		    ...
		    {fake_InquireDER_output_for_task_InformUserOfPowerStatus expectedOutputfake_InquireDER_output_for_task_InformUserOfPowerStatus=		    
		     new fake_InquireDER_output_for_task_InformUserOfPowerStatus(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputfake_InquireDER_output_for_task_InformUserOfPowerStatus,TaskOperations.CreateWF));
            }
         ...
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new InquireDERTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
   ...
   sg.setState("pending");
	...
	Vector evetns=null;
     ...
     app=DERSensorInit.createInstance(this);
	 //app.registerOwner(this);
	 	    
     this.getAM().addApplication("DERSensor",app);        
	 events=new Vector();
	 actions=new Vector();

	 if (getGraphics()!=null)
	  getGraphics().addApplication("DERSensor", events,actions);    

     //Initial applications assigned to the agent	  
   
     
     app=DERGUIInit.createInstance(this);
	 //app.registerOwner(this);
	 	    
     this.getAM().addApplication("DERGUI",app);        
	 events=new Vector();
	 actions=new Vector();

	 if (getGraphics()!=null)
	  getGraphics().addApplication("DERGUI", events,actions);    
