...
import ingenias.exception.NotFound;
  ...
  public class UserInteraction0DefaultCommControl extends DefaultCommControl{
  ...
  public UserInteraction0DefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("fake_RequirePowerInformation_output_for_task_InquireDER");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("fake_RequirePowerInformation_output_for_task_InquireDER");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InteractionUnit0") &&
		 ...
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"fake_RequirePowerInformation_output_for_task_InquireDER").isEmpty();
         ...
         if (allexist && true){
           ...
           sb.removeState("InteractionUnit0");
	   	   ...
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"fake_RequirePowerInformation_output_for_task_InquireDER");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("fake_RequirePowerInformation_output_for_task_InquireDER");
	       lockProcessed("fake_RequirePowerInformation_output_for_task_InquireDER");
           //MainInteractionManager.log("Removing lock fake_RequirePowerInformation_output_for_task_InquireDER",this.getAgent().getLocalName()+"-"+sb.getConversationID());
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for InteractionUnit1")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("InteractionUnit1")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for InteractionUnit1");           
    	   //try {
