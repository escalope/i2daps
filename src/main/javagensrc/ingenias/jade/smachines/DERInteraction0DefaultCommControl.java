...
import ingenias.exception.NotFound;
  ...
  public class DERInteraction0DefaultCommControl extends DefaultCommControl{
  ...
  public DERInteraction0DefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("fake_InquireDER_output_for_task_InformUserOfPowerStatus");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("fake_InquireDER_output_for_task_InformUserOfPowerStatus");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InteractionUnit1") &&
    	 sb.isState("InteractionUnit1")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"fake_InquireDER_output_for_task_InformUserOfPowerStatus").isEmpty();
         ...
         if (allexist && true){
           ...
           sb.removeState("InteractionUnit1");
	   	   ...
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"fake_InquireDER_output_for_task_InformUserOfPowerStatus");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("fake_InquireDER_output_for_task_InformUserOfPowerStatus");
	       lockProcessed("fake_InquireDER_output_for_task_InformUserOfPowerStatus");
           //MainInteractionManager.log("Removing lock fake_InquireDER_output_for_task_InformUserOfPowerStatus",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("endInteractionUnit1");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for InteractionUnit0")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("InteractionUnit0")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for InteractionUnit0");           
    	   //try {
		   ...
		   futureStates.add("InteractionUnit1");
