...
public class DERInteraction0StateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public DERInteraction0StateBehavior( String agentName,
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "Interaction0","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("InteractionUnit0") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if ("1".equals("n")){
         try{
          Vector<AID> receivers=this.getActor("User");    
  ...
  if (this.isState("InteractionUnit1")) {
     ...
     try {
      ...
      if (this.getDCC().notifyMessageSent("InteractionUnit1",optionsA,this)){
           ...
           "InteractionUnit1","Interaction0",this.getContentForNextMessage(), this.getPlayedRole());
           getTimeout().stop();
